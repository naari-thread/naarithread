...
import { isProductCategorySlug } from "@/lib/product-taxonomy";
...
export default async function CategoryProductsPage({ params }: CategoryProductsPageProps): Promise<ReactElement> {
  ...
  try {
    [initialProducts, searchIndex] = await Promise.all([
      listProductsPageFromCollection({ limit: 24, category }).then((page) => page.products),
      readProductSearchIndex(),
    ]);
  } catch (error) {
    console.error("[category-products-page] Failed to load cached products catalog:", error);
    hasFetchError = true;
  }
